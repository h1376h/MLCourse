...
def plot_likelihood_function(samples, save_path=None):
    ...
    test_points = [0.1, 0.2, 0.3, 0.4, p_mle, 0.6, 0.7, 0.8, 0.9]
...
def create_confidence_interval_visualization(samples, save_path=None):
    """Create a visualization of the MLE with confidence interval."""
    n = len(samples)
    p_mle = calculate_mle(samples)
    ...
    ci_lower = max(0, p_mle - z * se)
    ci_upper = min(1, p_mle + z * se)
    ...
    p_range = np.linspace(0.001, 0.999, 1000)
    likelihoods = [compute_likelihood(p, samples) for p in p_range]
    scaled_likelihoods = likelihoods / max(likelihoods) * 0.8  # Scale for visualization
    
    ax.plot(p_range, scaled_likelihoods, 'b-', linewidth=2, label='Scaled Likelihood')
    
    # Plot MLE and confidence interval
    ax.axvline(x=p_mle, color='red', linestyle='-', linewidth=2,
              label=f'MLE θ̂ = {p_mle:.4f}')
    
    ax.axvspan(ci_lower, ci_upper, alpha=0.2, color='green',
              label=f'95% CI: ({ci_lower:.4f}, {ci_upper:.4f})')
    
    # Plot points
    ax.plot([p_mle], [0.5], 'ro', markersize=10)
    ax.plot([ci_lower, ci_upper], [0.5, 0.5], 'go', markersize=8)
    
    ax.set_xlabel('θ')
    ax.set_ylabel('Scaled Likelihood')
    ax.set_title('MLE with 95% Confidence Interval')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    print(f"Maximum Likelihood Estimate: θ̂ = {p_mle:.4f}")
    print(f"Standard Error: SE(θ̂) = √(θ̂(1-θ̂)/n) = √({p_mle:.4f}×{1-p_mle:.4f}/{n}) = {se:.4f}")
    print(f"95% Confidence Interval: θ̂ ± 1.96×SE = {p_mle:.4f} ± {z*se:.4f} = ({ci_lower:.4f}, {ci_upper:.4f})")
    print(f"Interpretation: We are 95% confident that the true value of θ lies between {ci_lower:.4f} and {ci_upper:.4f}")
    
    # Calculate probability of future outcomes
    print("\nPredicted probabilities using MLE:")
    print(f"P(X=0|θ̂={p_mle:.4f}) = {1-p_mle:.4f}")
    print(f"P(X=1|θ̂={p_mle:.4f}) = {p_mle:.4f}")
    
    # Calculate asymptotic properties
    print("\nAsymptotic properties:")
    print(f"- The MLE is consistent: As n increases, θ̂ converges to the true value")
    print(f"- The MLE is asymptotically normal: √n(θ̂-θ) → N(0,θ(1-θ))")
    print(f"- The MLE is asymptotically efficient: It achieves the Cramér-Rao lower bound asymptotically")

def main():
    """Main function to generate all visualizations for Question 23."""
    # Create directory for saving images
    script_dir = os.path.dirname(os.path.abspath(__file__))
    images_dir = os.path.join(os.path.dirname(script_dir), "Images")
    save_dir = os.path.join(images_dir, "L2_4_Quiz_23")
    os.makedirs(save_dir, exist_ok=True)
    
    print("Generating visualizations for Question 23 of the L2.4 MLE quiz...")
    
    # Problem parameters: samples from the Bernoulli distribution
    # X_1 = 1, X_2 = 0, X_3 = 1, X_4 = 1, X_5 = 0, X_6 = 1, X_7 = 1, and X_8 = 0
    samples = np.array([1, 0, 1, 1, 0, 1, 1, 0])
    ...
    # 5. Create confidence interval visualization
    create_confidence_interval_visualization(samples,
                                          save_path=os.path.join(save_dir, "confidence_interval.png"))
    print("4. Confidence interval visualization created")
    ...
    # Additional statistics about the model
    print("\nAdditional statistics:")
    print(f"Variance of the MLE: {mle * (1-mle) / n:.6f}")
    print(f"Coefficient of Variation: {np.sqrt(mle * (1-mle) / n) / mle:.6f}")
    
    # Fisher Information
    fisher_info = n / (mle * (1-mle))
    print(f"\nFisher Information: I(θ) = n/(θ(1-θ)) = {n}/({mle}×{1-mle}) = {fisher_info:.6f}")
    print(f"Cramér-Rao Lower Bound: 1/I(θ) = {1/fisher_info:.6f}")
    
    print("\nThis completes the analysis for Question 23.")
