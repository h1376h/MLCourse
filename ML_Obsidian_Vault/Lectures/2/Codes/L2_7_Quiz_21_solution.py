...
print(f"From the graph E[Y|X=x] = 1.0 + 0.75x, we substitute x = 2")
print(f"MMSE Estimate = 1.0 + 0.75(2) = {mmse_estimate:.4f}")
...
plt.scatter([2], [mmse_estimate], color='red', s=100, 
            label=f'MMSE Estimate at X=2: y = {mmse_estimate:.2f}')
...
plt.text(0.5, 0.7, f'ML Estimate ({ml_estimate:.2f}): Maximizes the Likelihood function f(y|X=2)', 
         horizontalalignment='center', fontsize=12)
plt.text(0.5, 0.5, f'MAP Estimate ({map_estimate:.2f}): Maximizes the Posterior ∝ Likelihood × Prior', 
         horizontalalignment='center', fontsize=12)
plt.text(0.5, 0.3, f'MMSE Estimate ({mmse_estimate:.2f}): Expected value of Y given X=2 (E[Y|X=2])', 
         horizontalalignment='center', fontsize=12)
plt.text(0.5, 0.1, 'ML and MAP are close because they find peaks in distributions, while MMSE considers the entire distribution',
         horizontalalignment='center', fontsize=12)
plt.axis('off')
plt.tight_layout()
...
print("- For multimodal or asymmetric distributions, the mode and mean can differ substantially")
print(f"- Here, the MMSE estimate ({mmse_estimate:.2f}) is significantly higher than the MAP estimate ({map_estimate:.2f})")
print(f"  because of the bimodal nature of the likelihood and the corresponding posterior distribution")
print("")
...
print("- In this case, the bimodal nature of the likelihood and decreasing prior create a posterior")
print("  with different mode (MAP) and mean (MMSE) values") 