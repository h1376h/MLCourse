...
def create_solution_summary():
    """Create a summary visualization of the entire solution process."""
    # Print summary to console
    print("\nSolution Summary:")
    print("----------------")
    print(f"Naive Bayes model with the following parameters:")
    print(f"- P(X1=1|Y=0) = {p_x1_given_y0}")
    print(f"- P(X1=1|Y=1) = {p_x1_given_y1}")
    print(f"- P(X2=1|Y=0) = {p_x2_given_y0}")
    print(f"- P(X2=1|Y=1) = {p_x2_given_y1}")
    print(f"\nGiven that the likelihood of samples {{1,0,1}} and {{0,1,0}} is 1/180,")
    print(f"we found that w₁ = P(Y=1) = {sp.simplify(solutions[0])} ≈ {w1_value}")
    print(f"\nVerification: {p_sample1_numeric} * w₁ * {p_sample2_numeric} * (1-w₁) = 1/180")
    print(f"Substituting w₁ = {w1_value}: {p_sample1_numeric * w1_value * p_sample2_numeric * (1-w1_value):.8f} ≈ {1/180:.8f}")
    
    # Create simplified visualization
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    # 1. Naive Bayes Model (top-left)
    ax1 = axes[0, 0]
    G = nx.DiGraph()
    G.add_node("Y", pos=(0, 0))
    G.add_node("X1", pos=(-1, -1.5))
    ...
    nx.draw_networkx_nodes(G, pos, node_size=2000, 
                         node_color=['#3498db', '#2ecc71', '#2ecc71'], ax=ax1)
    nx.draw_networkx_edges(G, pos, width=2, arrowsize=20, ax=ax1)
    nx.draw_networkx_labels(G, pos, font_size=14, font_weight='bold', ax=ax1)
    ax1.set_title("Naive Bayes Graphical Model", fontsize=14)
    ax1.axis('off')
    
    # 2. Joint Probabilities (top-right)
    ax2 = axes[0, 1]
    w1_values = np.linspace(0, 1, 100)
    sample1_probs = p_sample1_numeric * w1_values
    sample2_probs = p_sample2_numeric * (1 - w1_values)
    ...
    # 3. Quadratic Equation (bottom-left)
    ax3 = axes[1, 0]
    w1_sym = sp.Symbol('w_1')
    expr = sp.lambdify(w1_sym, quadratic_eq, 'numpy')
    y_values = expr(w1_values)
    
    ax3.plot(w1_values, y_values, 'b-', linewidth=2, label='Quadratic Expression')
    ...
    for sol in solutions_numeric:
        if 0 <= sol <= 1:  # Only mark valid solutions
            ax3.plot(sol, 0, 'ro', markersize=8)
    
    # Add shaded region for valid probability range
    ax3.axvspan(0, 1, alpha=0.2, color='g', label='Valid Probability Range')
    
    ax3.grid(True, alpha=0.3)
    ax3.set_xlabel('w₁ = P(Y=1)', fontsize=12)
    ax3.set_ylabel('Expression Value', fontsize=12)
    ax3.set_title('Quadratic Equation Solution', fontsize=14)
    ax3.legend()
    
    # 4. Verification (bottom-right)
    ax4 = axes[1, 1]
    
    # Create a pie chart for the prior distribution
    labels = ['P(Y=0)', 'P(Y=1)']
    sizes = [1-w1_value, w1_value]
    colors = ['#e74c3c', '#3498db']
    explode = (0, 0.1)  # explode the 2nd slice (P(Y=1))
    
    ax4.pie(sizes, explode=explode, labels=labels, colors=colors, autopct='%1.1f%%',
          shadow=True, startangle=90)
    ax4.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle
    ax4.set_title('Prior Distribution', fontsize=14)
    
    plt.tight_layout()
    filename = os.path.join(save_dir, "naive_bayes_solution.png")
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Solution summary visualization saved to: {filename}")

# Create the solution summary visualization
