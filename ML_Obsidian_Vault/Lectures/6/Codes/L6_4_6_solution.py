...
print("     * If $R(T) = 0.2$ and $|T| = 20$, then $C(T) = 0.2 + 0.1 \\times 20 = 2.2$")
print("     * If $R(T) = 0.25$ and $|T| = 10$, then $C(T) = 0.25 + 0.1 \\times 10 = 1.25$")
...
print("     * $R_{bias}(T) \\approx 0.3$ (high due to underfitting)")
print("     * $R_{var}(T) \\approx 0.05$ (low due to simple model)")
print("     * $R_{noise}(T) \\approx 0.02$ (low sensitivity to noise)")
print("     * Total error: $0.3 + 0.05 + 0.02 = 0.37$")
print("     * Noise robustness: $\\frac{R_{noise}(T)}{R_{test}(T)} = \\frac{0.02}{0.37} = 5.4\\%$")
...
print("     * $R_{bias}(T) \\approx 0.15$ (medium due to moderate complexity)")
print("     * $R_{var}(T) \\approx 0.12$ (medium due to moderate flexibility)")
print("     * $R_{noise}(T) \\approx 0.08$ (medium sensitivity to noise)")
print("     * Total error: $0.15 + 0.12 + 0.08 = 0.35$")
print("     * Noise robustness: $\\frac{R_{noise}(T)}{R_{test}(T)} = \\frac{0.08}{0.35} = 22.9\\%$")
...
print("     * $R_{bias}(T) \\approx 0.08$ (low due to high flexibility)")
print("     * $R_{var}(T) \\approx 0.18$ (high due to complex model)")
print("     * $R_{noise}(T) \\approx 0.15$ (high sensitivity to noise)")
print("     * Total error: $0.08 + 0.18 + 0.15 = 0.41$")
print("     * Noise robustness: $\\frac{R_{noise}(T)}{R_{test}(T)} = \\frac{0.15}{0.41} = 36.6\\%$")
...
print("1. Pre-pruning: $5.4\\%$ (most robust)")
print("2. REP: $22.9\\%$ (moderately robust)")
print("3. CCP: $36.6\\%$ (least robust)")
...
print("     * If $n_{val} = 0.2n$ and $|T| \\approx 100$:")
print("       Training: $O(nd \\times 100) + O(100 \\times 0.2n) = O(100nd + 20n)$")
print("     * Prediction: $O(\\log |T|) = O(\\log 100) = O(6.6)$")
print("     * Memory: $O(|T|) = O(100)$ nodes")
...
print("     * If $|T| \\approx 100$:")
print("       Training: $O(nd \\times 100) + O(100^2 \\times \\log 100) = O(100nd + 10000 \\times 4.6)$")
print("       Training: $O(100nd + 46000)$")
print("     * Prediction: $O(\\log |T|) = O(\\log 100) = O(6.6)$")
print("     * Memory: $O(|T|) = O(100)$ nodes")
...
print("1. Pre-pruning: $O(8nd) = O(8 \\times 1000 \\times 10) = O(80,000)$")
print("2. REP: $O(100nd + 20n) = O(1,000,000 + 20,000) = O(1,020,000)$")
print("3. CCP: $O(100nd + 46,000) = O(1,000,000 + 46,000) = O(1,046,000)$")

print("\nSpeed ratios:")
print("REP vs Pre-pruning: $\\frac{1,020,000}{80,000} = 12.75\\times$ slower")
print("CCP vs Pre-pruning: $\\frac{1,046,000}{80,000} = 13.08\\times$ slower")
print("CCP vs REP: $\\frac{1,046,000}{1,020,000} = 1.025\\times$ slower")
...
print("     * Interpretability score: $1.5 \\times \\log(8) = 1.5 \\times 2.08 = 3.12$")
...
print("   - Mathematical reasoning:")
print("     * Typical depth: $D(T) = \\frac{5 + 8}{2} = 6.5$")
print("     * Typical leaves: $L(T) = \\frac{15 + 50}{2} = 32.5$")
print("     * Interpretability score: $6.5 \\times \\log(32.5) = 6.5 \\times 3.48 = 22.62$")
print("     * Rule complexity: $\\text{complexity}(T) = 32.5$ rules")
...
print("   - Mathematical reasoning:")
print("     * Typical depth: $D(T) = \\frac{4 + 6}{2} = 5$")
print("     * Typical leaves: $L(T) = \\frac{10 + 30}{2} = 20$")
print("     * Interpretability score: $5 \\times \\log(20) = 5 \\times 3.0 = 15.0$")
print("     * Rule complexity: $\\text{complexity}(T) = 20$ rules")
...
print("1. Pre-pruning: $3.12$ (most interpretable)")
print("2. CCP: $15.0$ (moderately interpretable)")
print("3. REP: $22.62$ (least interpretable)")

print("\nMathematical Insight:")
print("$I(T) \\propto D(T) \\times \\log(L(T))$")
print("Pre-pruning minimizes both depth and leaf count")
print("Interpretability ratio: $\\frac{22.62}{3.12} = 7.25\\times$ worse for REP vs Pre-pruning")
...
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 15^{0.5} \\cdot 3^{1.0}$")
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 3.87 \\cdot 3 = 11.61k\\varepsilon(x)$")
print("     * As $\\varepsilon(x)$ increases, error grows linearly")
print("     * Performance degradation: $\\frac{\\Delta R_{noise}}{\\Delta \\varepsilon} = 11.61k$ (constant)")
...
print("     * Variable size: $|T| \\approx 60$ (from previous analysis)")
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 60^{0.5} \\cdot 6.5^{1.0}$")
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 7.75 \\cdot 6.5 = 50.38k\\varepsilon(x)$")
print("     * As $\\varepsilon(x)$ increases, error grows linearly but faster")
print("     * Performance degradation: $\\frac{\\Delta R_{noise}}{\\Delta \\varepsilon} = 50.38k$ (4.34× worse than pre-pruning)")
...
print("     * Variable size: $|T| \\approx 20$ (from previous analysis)")
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 20^{0.5} \\cdot 5^{1.0}$")
print("     * Noise sensitivity: $R_{noise}(T, x) = k \\cdot \\varepsilon(x) \\cdot 4.47 \\cdot 5 = 22.36k\\varepsilon(x)$")
print("     * As $\\varepsilon(x)$ increases, error grows linearly")
print("     * Performance degradation: $\\frac{\\Delta R_{noise}}{\\Delta \\varepsilon} = 22.36k$ (1.93× worse than pre-pruning)")
...
print("1. Pre-pruning: $11.61k$ (most stable)")
print("2. CCP: $22.36k$ (moderately stable)")
print("3. REP: $50.38k$ (least stable)")

print("\nPerformance degradation ratios:")
print("REP vs Pre-pruning: $\\frac{50.38k}{11.61k} = 4.34\\times$ worse")
print("CCP vs Pre-pruning: $\\frac{22.36k}{11.61k} = 1.93\\times$ worse")
print("REP vs CCP: $\\frac{50.38k}{22.36k} = 2.25\\times$ worse")
...
print("   - Mathematical reasoning:")
print("     * Prediction: $t_{pred} = 3 \\times t_{node} \\approx 3 \\times 0.001ms = 0.003ms$")
print("     * Training: $t_{train} = 8nd \\times t_{split} \\approx 8 \\times 1000 \\times 10 \\times 0.001ms = 80ms$")
print("     * Update: $t_{update} = t_{train} = 80ms$")
print("     * Throughput: $\\text{throughput} = \\frac{1000}{0.003} = 333,333$ req/s")
print("     * Latency: $0.003ms \\ll 100ms$ ✓")
print("     * Update time: $80ms < 100ms$ ✓")
...
print("   - Choice: POOR")
print("   - Mathematical reasoning:")
print("     * Prediction: $t_{pred} = \\log(100) \\times t_{node} \\approx 6.6 \\times 0.001ms = 0.0066ms$")
print("     * Training: $t_{train} = nd|T| \\times t_{split} \\approx 1000 \\times 10 \\times 100 \\times 0.001ms = 1000ms$")
print("     * Update: $t_{update} = t_{train} = 1000ms$")
print("     * Throughput: $\\text{throughput} = \\frac{1000}{0.0066} = 151,515$ req/s")
print("     * Latency: $0.0066ms < 100ms$ ✓")
print("     * Update time: $1000ms > 100ms$ ✗ (too slow)")
print("     * Problem: Updates take 10× longer than allowed")
...
print("   - Choice: POOR")
print("   - Mathematical reasoning:")
print("     * Prediction: $t_{pred} = \\log(20) \\times t_{node} \\approx 4.3 \\times 0.001ms = 0.0043ms$")
print("     * Training: $t_{train} = nd|T|^2 \\times t_{split} \\approx 1000 \\times 10 \\times 20^2 \\times 0.001ms = 4000ms$")
print("     * Update: $t_{update} = t_{train} = 4000ms$")
print("     * Throughput: $\\text{throughput} = \\frac{1000}{0.0043} = 232,558$ req/s")
print("     * Latency: $0.0043ms < 100ms$ ✓")
print("     * Update time: $4000ms > 100ms$ ✗ (40× too slow)")
...
print("2. REP: $\\frac{1000ms}{80ms} = 12.5\\times$ slower")
print("3. CCP: $\\frac{4000ms}{80ms} = 50\\times$ slower")

print("\nThroughput comparison:")
print("1. Pre-pruning: $333,333$ req/s (highest)")
print("2. CCP: $232,558$ req/s (medium)")
print("3. REP: $151,515$ req/s (lowest)")
...
print("Required sample size: $n = \\frac{2(t_{\\alpha/2} + t_{\\beta})^2}{d^2} \\approx 64$ per group")
print("With 3 methods: total $n = 64 \\times 3 = 192$ runs")
