...
$$K_{string}(s_1, s_2) = \phi_k(s_1)^T \phi_k(s_2) = \sum_{w \in \Sigma^k} \phi_k(s_1)[w] \cdot \phi_k(s_2)[w]$$
...
$\text{k-mers}(s_1) = \{\text{ATC}, \text{TCG}, \text{CGA}, \text{GAT}, \text{ATC}, \text{TCG}\}$

For $s_2 = \text{ATCGTTCG}$ with $k=3$:
- Position 0: ATC
- Position 1: TCG
- Position 2: CGT
- Position 3: GTT
...
$\text{k-mers}(s_2) = \{\text{ATC}, \text{TCG}, \text{CGT}, \text{GTT}, \text{TTC}, \text{TCG}\}$

**Step 1.2: Count k-mer frequencies**
For $s_1$: $\phi_k(s_1) = \{\text{ATC}: 2, \text{TCG}: 2, \text{CGA}: 1, \text{GAT}: 1\}$
For $s_2$: $\phi_k(s_2) = \{\text{ATC}: 1, \text{TCG}: 2, \text{CGT}: 1, \text{GTT}: 1, \text{TTC}: 1\}$

**Step 1.3: Compute kernel value**
$$K_{string}(s_1, s_2) = \sum_{w} \phi_k(s_1)[w] \cdot \phi_k(s_2)[w]$$

$$= \phi_k(s_1)[\text{ATC}] \cdot \phi_k(s_2)[\text{ATC}] + \phi_k(s_1)[\text{TCG}] \cdot \phi_k(s_2)[\text{TCG}]$$
$$+ \phi_k(s_1)[\text{CGA}] \cdot \phi_k(s_2)[\text{CGA}] + \phi_k(s_1)[\text{GAT}] \cdot \phi_k(s_2)[\text{GAT}]$$
$$+ \phi_k(s_1)[\text{CGT}] \cdot \phi_k(s_2)[\text{CGT}] + \phi_k(s_1)[\text{GTT}] \cdot \phi_k(s_2)[\text{GTT}]$$
$$+ \phi_k(s_1)[\text{TTC}] \cdot \phi_k(s_2)[\text{TTC}]$$
...
**Mathematical Definition:**
$$K_{graph}(G_1, G_2) = K_{degree}(G_1, G_2) + K_{substructure}(G_1, G_2)$$

where:
- $K_{degree}(G_1, G_2) = \sum_{d} \text{count}_d(G_1) \cdot \text{count}_d(G_2)$ (degree sequence similarity)
- $K_{substructure}(G_1, G_2)$ measures common substructures (triangles, cliques)
...
For $G_1$ (Triangle):
- Node 0: degree = 2 (connected to nodes 1, 2)
- Node 1: degree = 2 (connected to nodes 0, 2)
- Node 2: degree = 2 (connected to nodes 0, 1)
- Degree sequence: $[2, 2, 2]$
- Degree counts: $\text{count}_2(G_1) = 3, \text{count}_d(G_1) = 0$ for $d \neq 2$

For $G_2$ (Square):
- Node 0: degree = 2 (connected to nodes 1, 3)
- Node 1: degree = 2 (connected to nodes 0, 2)
- Node 2: degree = 2 (connected to nodes 1, 3)
- Node 3: degree = 2 (connected to nodes 0, 2)
- Degree sequence: $[2, 2, 2, 2]$
- Degree counts: $\text{count}_2(G_2) = 4, \text{count}_d(G_2) = 0$ for $d \neq 2$

**Step 2.2: Compute degree similarity**
$$K_{degree}(G_1, G_2) = \sum_{d} \text{count}_d(G_1) \cdot \text{count}_d(G_2)$$
...
Triangle count in $G_1$: 1 (the entire graph is a triangle)
Triangle count in $G_2$: 0 (no triangles in a square)

$$K_{substructure}(G_1, G_2) = \min(\text{triangles}(G_1), \text{triangles}(G_2)) = \min(1, 0) = 0$$

**Step 2.4: Final kernel value**
$$K_{graph}(G_1, G_2) = K_{degree}(G_1, G_2) + K_{substructure}(G_1, G_2) = 12 + 0 = 12$$
...
**Mathematical Definition:**
$$K_{ts}(x, y) = \exp\left(-\sigma \left(1 - \max_{\tau} \text{corr}(x, y_\tau)\right)^2\right)$$
...
$$K_{ts}(x, y) = \exp\left(-\sigma (1 - \rho_{max})^2\right)$$

**Example calculation:**
For $x = \sin(t)$ and $y = \sin(t + \pi/4)$ with $\sigma = 1$:
- Maximum correlation occurs at $\tau$ corresponding to phase shift $\pi/4$
- $\rho_{max} \approx 0.999$ (nearly perfect correlation when aligned)
- $K_{ts}(x, y) = \exp(-1 \cdot (1 - 0.999)^2) = \exp(-0.000001) \approx 0.999$
...
If $K(s_1, s_1) = 25$, $K(s_2, s_2) = 18$, and $K(s_1, s_2) = 6$:
